    ...
    for event in data_root.iterdir():
        if event.is_dir():
            ...
            for file_path in event.iterdir():
                if 'img.tif' in file_path.name:
                    create_vv_and_vh_tifs(file_path)
                # if dem reproject to match higher resolution
                if 'vv.tif' in file_path.name:
                    reference_resolution = extract_resolution(file_path)
                    print(f"---Resolution: {reference_resolution}")
                if 'dem.tif' in file_path.name:
                    match_resolutions(reference_resolution  , file_path)
...
#TODO CHECK THIS !!!!!
def match_resolutions(reference_path, target_path, output_path):
    ...
    with rxr.open_rasterio(reference_path) as reference_layer:
        ...
        with rxr.open_rasterio(target_path) as target_layer:
            ...
            resampled_layer.rio.to_raster(output_path)

    print(f"Resampled raster saved to: {output_path}")

def extract_resolution(reference_path):
    """Extract the resolution from the reference raster."""
    with rxr.open_rasterio(reference_path) as reference_layer:
        resolution = reference_layer.rio.resolution()  # Get resolution (x, y)
    return resolution

def main():
    # TODO add RTC functionality
    data_root = Path(r"Z:\1NEW_DATA\1data\2interim\TESTS\sample_s1s24326")
    datacube = make_datacube(data_root)

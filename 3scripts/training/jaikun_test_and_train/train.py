...
def main(test, reproduce):
    ...
    if not reproduce:
        ...
        with wandb.init(project=project, job_type="data-process", name='load-data') as run:
            ...
            data_artifact.add_reference(name="train_list", uri=str(train_list))
        ...
        with wandb.init(project=project, job_type="reproduce", name='reproduce') as run:
    ...
    if not test:
        floodsegment = SegmentFlood(model)
        trainer.fit(floodsegment, train_dataloaders=train_dl, val_dataloaders=val_dl)
        # trainer.test(model=floodsegment, dataloaders=test_dl, ckpt_path='best')
